...
# df = pd.read_csv("c_pending.csv")
#
# years = list(range(2000, 2018))
# years = [str(year) for year in years]
#
# application = ["decisions", "pending"]
# colors = ["#c9d9d3", "#718dbf"]
#
# s31 = ColumnDataSource(df)
#
# p1 = figure(plot_height=500, title="Asylum Applications", tooltips="$name @Country: @$name")
# p1.vbar_stack(application, x='Year', width=0.4, color=colors, source=s31, legend_label=application)
# p1.line(x='Year', y='applied', source=s31)
#
# p1.y_range.start = 0
...
# PLOT 4: Immigration
df = pd.read_csv("c_immigration.csv")
df['FromColombia'] = df['FromColombia'].pct_change()
df['ToColombia'] = df['ToColombia'].pct_change()

s41 = ColumnDataSource(df)

p41 = figure(plot_width=480, plot_height=280, title="Migrant Stock % Increase", toolbar_location=None)
#p41.sizing_mode = 'scale_width'
p41.outline_line_alpha = 0

p41.line(x='Year', y='FromColombia', color='blue', source=s41, line_width=2)
p41.line(x='Year', y='ToColombia', color='red', source=s41, line_width=2)
