...
d3.csv("https://raw.githubusercontent.com/meenurajapandian/CGDVColombia/master/datasets/data_groupedbar.csv", function(error, mydata) {
  if (error) throw error;
    ...
    for (var i =0; i < mydata.length; i++){
        for (var j = 0 ; j < 2; j++) {
            if(j%2 == 0){
                findata[i] = {name: mydata[i].Age, values: [{x: 0, y: +mydata[i].Male, y0: 0}, {x: 1, y:+mydata[i].Female, y0: 0} ] }
            }
        // findata[i].values = []
...
var seriesNames = ["Below_17", "Below_29", "Below_39", "Below_49", "Below_59", "Below_69", "Over_70"],
    numSamples = 2,
